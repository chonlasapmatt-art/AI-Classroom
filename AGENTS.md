...
That is enough to build screens, change behaviour, run all 129 test files and see the result. Most work on
...
npm run test        # vitest, 129 test files
npm run build
```
...
There are 92; the last is `202609110001`. Replacing a function in a *new* migration is the repair
...
supabase/
  migrations/     92 immutable migrations
