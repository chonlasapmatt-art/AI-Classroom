...
That is enough to build screens, change behaviour, run all 127 test files and see the result. Most work on
...
npm run test        # vitest, 127 test files
npm run build
```
...
There are 91; the last is `202609100005`. Replacing a function in a *new* migration is the repair
...
supabase/
  migrations/     91 immutable migrations
