...
That is enough to build screens, change behaviour, run all 135 test files and see the result. Most work on
...
npm run test        # vitest, 135 test files
npm run build
```
...
There are 93; the last is `202609120001`. Replacing a function in a *new* migration is the repair
...
supabase/
  migrations/     93 immutable migrations
