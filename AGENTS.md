...
That is enough to build screens, change behaviour, run all 137 test files and see the result. Most work on
...
npm run test        # vitest, 137 test files
npm run build
```
...
There are 95; the last is `202609120003`. Replacing a function in a *new* migration is the repair
...
supabase/
  migrations/     95 immutable migrations
