...
That is enough to build screens, change behaviour, run all 113 test files and see the result. Most work on
...
npm run test        # vitest, 113 test files
npm run build
```
...
There are 86; the last is `202609090011`. Replacing a function in a *new* migration is the repair
...
supabase/
  migrations/     86 immutable migrations
