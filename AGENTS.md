...
That is enough to build screens, change behaviour, run all 120 test files and see the result. Most work on
...
npm run test        # vitest, 120 test files
npm run build
```
...
There are 87; the last is `202609100001`. Replacing a function in a *new* migration is the repair
...
supabase/
  migrations/     87 immutable migrations
