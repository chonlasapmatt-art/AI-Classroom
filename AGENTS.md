...
That is enough to build screens, change behaviour, run all 140 test files and see the result. Most work on
...
npm run test        # vitest, 140 test files
npm run build
```
...
There are 96; the last is `202609130001`. Replacing a function in a *new* migration is the repair
...
supabase/
  migrations/     96 immutable migrations
