...
That is enough to build screens, change behaviour, run all 110 test files and see the result. Most work on
...
npm run test        # vitest, 110 test files
npm run build
```
...
There are 85; the last is `202609090010`. Replacing a function in a *new* migration is the repair
...
supabase/
  migrations/     85 immutable migrations
