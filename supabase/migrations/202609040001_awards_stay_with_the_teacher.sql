-- A score belongs to a teacher who teaches the student, and to nobody else.
--
-- Two things were wrong with how the sync boundary decided this, and both were widenings that came
-- in with a change about something else.
--
-- The first: score writes were checked in one list with the submission, and that list admits
-- `student_owns_student_record`. Turning work in is a student's own act, so the exemption is right
-- for a submission and wrong for everything it was sitting beside — with it, a student's own device
-- could insert a test score, an activity score or a board award for themselves. Submissions keep
-- the exemption; marks do not.
--
-- The second: any teacher of the school could award any student. That was tolerable while a score
-- was typed on a gradebook screen the teacher had to reach through their own classes anyway, and it
-- stops being tolerable now that a board hands out points in one tap. A teacher awards the students
-- they teach.
--
-- `staff_can_award_student` is the one place that rule lives, and the score, the badge and both
-- kinds of mark all ask it. Everything else about the mutation — idempotency, conflicts, the audit
-- entry, the journal — is carried over from `202609020001` unchanged.

begin;

/**
 * Whether the caller may put a score or a badge on this student.
 *
 * An admin may, anywhere in their school. A teacher may where they teach: the student has to hold
 * an active enrollment in a class the teacher has access to, and when the award names a class it
 * has to be that class. A null class means "wherever this student is taught", which is what a badge
 * and a corrected score arrive as.
 */
create or replace function public.staff_can_award_student(p_school_id uuid, p_student_id uuid, p_class_id uuid)
returns boolean language sql stable security definer set search_path=public,pg_temp as $$
  select public.has_school_role(p_school_id,'admin')
      or (
        public.has_school_role(p_school_id,'teacher')
        and exists(
          select 1 from public.student_class_enrollments e
          where e.school_id=p_school_id and e.student_id=p_student_id
...
revoke all on function public.staff_can_award_student(uuid,uuid,uuid) from public,anon;
grant execute on function public.staff_can_award_student(uuid,uuid,uuid) to authenticated;

comment on function public.staff_can_award_student(uuid,uuid,uuid) is
  'True for an admin of the school, and for a teacher who teaches the student in the named class.';
...
begin
  ...
  if p_entity_type in ('assignment','activity','test') and not (public.has_school_role(p_school_id,'admin') or public.has_school_role(p_school_id,'teacher')) then raise exception 'FORBIDDEN' using errcode='42501'; end if;
