-- Only the teacher who owns a class's subject writes that subject's marks.
--
-- The screens have always said so: score entry is gated on `canManageAcademicItem`, which requires
-- the SUBJECT_OWNER of that class and subject. The server said something weaker -- it asked only
-- `staff_can_award_student`, which is satisfied by anybody on the room's staff list -- so the
-- maths teacher of a room could write a science mark for a child in it through the API even though
-- no screen offers the field. The school asked for the stricter rule; this is it, in the one place
-- that can enforce it.
--
-- Two things are deliberately left at room level. A mark whose parent activity or test carries no
-- subject has no owner to require, so it stays with the room rather than becoming unwritable by
-- anyone. And the child must still be enrolled in the room the mark is for, which is the check that
-- was already there and is kept as the outer arm.

/** True when the caller is the one active owner of this class's subject. */
create or replace function public.teacher_owns_class_subject(p_class_id uuid, p_subject_id uuid)
returns boolean language sql stable security definer set search_path to 'public','pg_temp'
as $fn$
  select p_subject_id is not null and exists(
    select 1 from public.class_teachers ct
    join public.teachers t on t.id = ct.teacher_id
    where ct.class_id = p_class_id and ct.subject_id = p_subject_id
      and ct.role_in_class = 'primary'
...
/** Whether this caller may write one mark: the right child, and the right subject. */
create or replace function public.can_write_subject_mark(
  p_school_id uuid, p_class_id uuid, p_subject_id uuid, p_student_id uuid
) returns boolean language sql stable security definer set search_path to 'public','pg_temp'
as $fn$
  select public.staff_can_award_student(p_school_id, p_student_id, p_class_id)
    and (
      public.has_school_role(p_school_id,'admin')
      or p_subject_id is null
      or public.teacher_owns_class_subject(p_class_id, p_subject_id)
    );
$fn$;

-- Reached only from the definer function below, never called by a device directly.
revoke all on function public.teacher_owns_class_subject(uuid, uuid) from public, anon, authenticated;
revoke all on function public.can_write_subject_mark(uuid, uuid, uuid, uuid) from public, anon, authenticated;

-- Everything else in the function below is the deployed definition, unchanged.

CREATE OR REPLACE FUNCTION public.apply_sync_mutation(p_school_id uuid, p_device_id uuid, p_idempotency_key text, p_request_hash text, p_entity_type text, p_entity_id uuid, p_operation sync_operation, p_payload jsonb, p_base_version integer)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public', 'pg_temp'
AS $function$
declare actor uuid:=auth.uid(); stored public.sync_idempotency%rowtype; device public.devices%rowtype; current_version integer; result jsonb; new_revision bigint; class_scope uuid; student_scope uuid; subject_scope uuid; critical boolean;
begin
  ...
  if p_entity_type='submission' and not (public.has_school_role(p_school_id,'admin') or public.has_school_role(p_school_id,'teacher') or public.student_owns_student_record(coalesce(student_scope,(p_payload->>'studentId')::uuid))) then raise exception 'FORBIDDEN' using errcode='42501'; end if;
  /*
   * Work belongs to a room, and so does the right to set it.
   *
   * This asked only whether the caller was a teacher of the school, with no comparison against the
   * room at all, so every room in the school was writable by every teacher in it. The room comes
   * from the existing row when there is one and from the payload when the work is being created --
   * the same pair the attendance guard above already uses.
   */
  if p_entity_type in ('assignment','activity','test') and not (
    public.has_school_role(p_school_id,'admin')
    or public.teacher_has_class_access(coalesce(class_scope,(p_payload->>'classId')::uuid))
  ...
  /*
   * A mark belongs to the teacher of that subject.
   *
   * This asked only whether the caller taught the child -- which every member of the room's staff
   * list does -- so the maths teacher could write a science mark for a child they share. The
   * screens have always refused that (`canManageAcademicItem` requires the subject owner); the
   * server did not, and the two now say the same thing.
   *
   * The subject comes from the parent activity or test, taken from the payload when the mark is
   * new and from the stored row when it is an edit. A mark with no subject on its parent has no
   * owner to require, so it stays with the room rather than becoming unwritable.
   */
  if p_entity_type in ('activity_score','test_score','score_event') then
    if p_entity_type = 'activity_score' then
      select a.class_id, a.subject_id into class_scope, subject_scope from public.activities a
      where a.id = coalesce((p_payload->>'activityId')::uuid,
        (select sc.activity_id from public.activity_scores sc where sc.id = p_entity_id));
    elsif p_entity_type = 'test_score' then
      select t.class_id, t.subject_id into class_scope, subject_scope from public.tests t
      where t.id = coalesce((p_payload->>'testId')::uuid,
        (select sc.test_id from public.test_scores sc where sc.id = p_entity_id));
    else
      class_scope := coalesce(class_scope,(p_payload->>'classId')::uuid);
      subject_scope := (p_payload->>'subjectId')::uuid;
    end if;
    if not public.can_write_subject_mark(
      p_school_id, class_scope, subject_scope,
      coalesce(student_scope,(p_payload->>'studentId')::uuid)
    ) then raise exception 'FORBIDDEN: SUBJECT_OWNER_REQUIRED' using errcode='42501'; end if;
  end if;
  ...
  if p_operation='delete' then
    ...
    case p_entity_type
      ...
      when 'submission' then
        insert into public.submissions(id,school_id,assignment_id,student_id,submitted_at,status,score,is_late,teacher_note,drive_url,version) values(p_entity_id,p_school_id,(p_payload->>'assignmentId')::uuid,(p_payload->>'studentId')::uuid,(p_payload->>'submittedAt')::timestamptz,p_payload->>'status',(p_payload->>'score')::numeric,coalesce((p_payload->>'isLate')::boolean,false),coalesce(p_payload->>'teacherNote',''),nullif(p_payload->>'driveUrl',''),1) on conflict(id) do update set submitted_at=excluded.submitted_at,status=excluded.status,score=excluded.score,is_late=excluded.is_late,teacher_note=excluded.teacher_note,drive_url=coalesce(excluded.drive_url,public.submissions.drive_url),updated_at=clock_timestamp(),server_updated_at=clock_timestamp(),version=public.submissions.version+1,deleted_at=null returning version into current_version;
      when 'activity' then
