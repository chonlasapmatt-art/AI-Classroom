-- Work, activities and tests belong to a room the teacher was actually given.
--
-- `apply_sync_mutation` gated every other entity on the room -- attendance and timetable entries on
-- `teacher_has_class_access`, marks and badges on `staff_can_award_student` -- but assignments,
-- activities and tests were gated on `has_school_role(school,'teacher')` alone. The class_id was
-- read only to take the version lock and was never compared against the caller, so any verified
-- teacher in the school could create or edit work in any room in the school, including rooms they
-- had never been put in charge of.
--
-- Nothing in the app offers that: every screen builds its class picker from the teacher's own staff
-- list. It was reachable only by a client that did not -- an old build, a script, a forged mutation
-- -- which is the case a server-side check exists for.
--
-- The rule now matches the one the rest of the table already follows: an administrator, or a
-- teacher on that room's staff list. Ownership of the *subject* is deliberately not required here.
-- The school's own screens let an advisor set work for their room, and requiring subject ownership
-- would take work away from people doing it today; that narrower question is the school's to
-- decide, not this migration's.
--
-- Everything else in the function is the deployed definition, unchanged.
