-- All school member accounts are managed by a school administrator. Public registration is no
-- longer part of the product, so this trusted function is the only path that binds an Auth user to
-- a teacher, student, or parent roster record.
...
begin
  if p_role not in ('teacher','student','parent','admin') or char_length(wanted)<2 then
...
begin
  ...
  insert into public.school_memberships(school_id,profile_id,role,status)
    values(p_school_id,p_profile_id,p_role,'active')
