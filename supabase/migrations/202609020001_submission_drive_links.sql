...
do $$
begin
  if not exists (select 1 from pg_constraint where conname = 'submissions_drive_url_check') then
    alter table public.submissions add constraint submissions_drive_url_check check (
      drive_url is null or drive_url ~ '^https://(drive\.google\.com|docs\.google\.com|drive\.usercontent\.google\.com)/'
    );
  end if;
end $$;
