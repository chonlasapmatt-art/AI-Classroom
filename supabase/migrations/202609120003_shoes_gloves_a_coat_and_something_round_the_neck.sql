-- Shoes, gloves, a coat and something round the neck — and what the server charges for them.
--
-- The customiser dressed a child from the waist up and the waist down and nothing else. Whatever
-- boot a pair of trousers happened to draw was the boot every figure in the school wore; there were
-- no gloves, no coat over the shirt, nothing at the throat. Four new drawers close that: nine pairs
-- of footwear, seven of handwear, eight outer garments and eight pieces of neckwear.
--
-- Thirty-one of them are priced, and a price the app knows but the server does not is a price nobody
-- pays: the request to wear a thing comes from the child's own browser, and "the client said it was
-- free" is not a permission check.
--
-- `avatar_trait_price` is replaced whole rather than patched, for the reason each replacement before
-- it gave: the function is a single `case` over the trait id, so there is nothing to add to. A
-- second function handling only the new ids would have to be consulted by everything that consults
-- this one, and the first caller to forget would hand out the expensive half of the wardrobe.
--
-- Every id that existed before keeps the price it had. The new ids are priced against the pieces
-- beside them — a pair of trainers at 40 where a plain hoodie is 40, plate sabatons at 110 where
-- plate legs are 130 — so that saving up for a whole outfit costs what saving up for its parts did.
--
-- The guard that validates a saved config needs no change: it checks a layer key against
-- `^[a-z][a-z_]{0,23}$`, which `footwear`, `handwear`, `outerwear` and `neckwear` already satisfy,
-- and prices each half of a trait id through this function.
...
as $fn$
  ...
  -- The base half: an eye shape, a haircut, a top, a boot, a pair of wings. Absent from here is
  -- free, and most of the wardrobe is absent from here on purpose — a shop where everything is
  -- locked is a shop that tells a new child they are dressed wrong.
  base_price as (
    select case split_part((select base from parts), '_', 1) || ':' ||
      ...
      when 'hand:fingerless' then 30
      when 'bottom:robehem' then 40
      when 'foot:trainer' then 40
      when 'fx:sparkle' then 40
      when 'hand:handwrap' then 40
      when 'neck:ribbonbow' then 40
      ...
      when 'neck:longscarf' then 50
      when 'outer:gilet' then 50
      when 'outer:harness' then 50
      ...
      when 'foot:buckleboot' then 60
      when 'front:lantern' then 60
      when 'fx:leaffall' then 60
      when 'hair:dreadlocks' then 60
      when 'neck:bellcollar' then 60
      when 'neck:pendant' then 60
      when 'outer:opencoat' then 60
      ...
      when 'hand:runeband' then 100
      when 'outer:overcoat' then 100
      ...
      when 'foot:hoverplate' then 130
      when 'fx:neonstreak' then 130
      when 'outer:circuitmantle' then 130
      when 'aura:frostring' then 140
      when 'back:angelwings' then 140
      when 'foot:thrusterboot' then 140
