...
export interface StudentQuizView {
  ...
  question: Omit<BoardQuestion, 'explanation'> | null;
...
export function secondsRemaining(
  deadline: string | null, serverTime: string, now = Date.now(), currentTime = Date.now()
): number | null {
  if (!deadline) return null;
  const offset = now - Date.parse(serverTime);
  const estimatedServerNow = currentTime - offset;
