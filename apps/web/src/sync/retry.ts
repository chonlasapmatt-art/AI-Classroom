...
// 409 is the server asking for a newer build: the work is fine, the app just has to be refreshed,
// and blocking every queued change behind that would make the person discard work that was correct.
export function isRetryableStatus(status: number): boolean { return status === 401 || status === 403 || status === 408 || status === 409 || status === 429 || status >= 500; }
